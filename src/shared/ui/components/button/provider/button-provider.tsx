...
export const ButtonProvider = (props: ButtonProvider.Props) => {
	const { children, color, size, radius, variant, disabled, loading, iconButton, animateBorder } = props;

	const contextValue: ButtonContext = React.useMemo(
		() => ({ color, size, variant, disabled, loading, iconButton, radius, animateBorder }),
		[color, size, variant, disabled, loading, iconButton, radius, animateBorder]
	);
